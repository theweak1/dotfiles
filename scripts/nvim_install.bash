...
echo -e "${YELLOW}Detected GLIBC version: $glibc_version${NC}"

# Function to download and install nvim
install_nvim() {
    echo -e "${YELLOW}Installing nvim...${NC}"
    sudo apt update
    ...
    if [[ $(echo "$glibc_version < 2.32" | bc -l) -eq 1 ]]; then
        echo -e "${YELLOW}Older glibc detected ($glibc_version). Using fallback Neovim build...${NC}"
        ...
        download_url=$(echo "$release_json" | jq -r --arg file "$file" '.assets[] | select(.name == $file) | .browser_download_url')

        if [[ -z "$download_url" ]]; then
            echo -e "${RED}Could not find a matching download for your architecture ($ARCH).${NC}"
            exit 1
        fi
    fi

    echo -e "${GREEN}Downloading Neovim from:${NC} $download_url"
...
if command -v "nvim" &> /dev/null; then
    echo -e "${GREEN}nvim is already installed.${NC}"
...
if fc-list | grep -i "$FONT_NAME" &> /dev/null; then
    ...
    echo -e "${YELLOW}Font $FONT_NAME is not installed. Installing...${NC}"
...
echo -e "\n${GREEN}<<< nvim setup finished successfully! >>>${NC}\n"
